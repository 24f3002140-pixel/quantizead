...
def validate_freeze_global(body):
    ...
    for c in candidates:
        ...
        if not isinstance(files, dict):
            return False
        # allow empty files – will be handled later

        if "loadable" not in c or not isinstance(c["loadable"], bool):
            return False
        if "calibrationDigest" not in c or not nonempty_string(c["calibrationDigest"]):
            return False
        if "tokenizerDigest" not in c or not nonempty_string(c["tokenizerDigest"]):
            return False
...
def do_freeze(body):
    ...
    if freeze_id in FREEZES:
        ...
        if canonical_equal(old["input"], body):
            return JSONResponse(status_code=200, content=deepcopy(old["response"]))
    ...
    for candidate in body["candidates"]:
        results.append(freeze_candidate(candidate, request_cal, request_tok, allowed))

    results.sort(key=lambda x: x["name"].encode("utf-8"))

    response = {"freezeId": freeze_id, "candidates": results}
    FREEZES[freeze_id] = {"input": deepcopy(body), "response": deepcopy(response)}
...
def evaluate_candidate(candidate, rows, required_slices):
    ...
    for required_name in required_slices:
        matching = [(label, pred) for label, slice_name, pred in valid_rows if slice_name == required_name]
        if matching:
...
def do_select(body):
    ...
    stored_names = [c["name"] for c in stored["response"]["candidates"]]
    submitted_names = [c.get("name") if isinstance(c, dict) else None for c in submitted_candidates]

    if (len(stored_names) != len(set(stored_names)) or
    ...
    for candidate in submitted_candidates:
        ...
        aggregate, slices, prediction_invalid = evaluate_candidate(
            candidate, rows, policy["requiredSlices"]
        )
    ...
    order_index = {name: i for i, name in enumerate(candidate_order)}
    results.sort(key=lambda x: (order_index.get(x["name"], len(candidate_order)), x["name"].encode("utf-8")))
    ...
    if admitted_results:
        winner = min(
            ...
            key=lambda r: (
                r["totalBytes"],
                float(r["latencyMs"]),
                order_index.get(r["name"], len(candidate_order)),
                r["name"].encode("utf-8"),
            ),
        )
        selected = winner["name"]
        winner_candidate = next(c for c in stored["response"]["candidates"] if c["name"] == selected)
...
async def quantize(request: Request):
    ...
    if (not isinstance(candidates, list) or len(candidates) == 0 or
        not isinstance(rows, list) or len(rows) == 0 or
        not isinstance(policy, dict) or
        "freezeId" not in body or "latencies" not in body):
        return error_response("INVALID_INPUT", 400)
...
@app.get("/")
async def root():
    return {"service": "quantize-admission", "status": "ok"}
